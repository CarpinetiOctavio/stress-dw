...
The legacy raw input (`data/raw/mental_health.csv` at tag `legacy-original`) has 292,364 records and 17 columns, and 5,202 nulls in `self_employed` according to the legacy ETL log. The Data Card reports about 292k rows, 17 columns and 5,202 missing values in `self_employed`. Whether the two files are byte-identical is pending a SHA-256 comparison.

| File | SHA-256 |
|------|---------|
| Legacy raw file (`data/raw/mental_health.csv`) | pending |
| Fresh Kaggle download (`Mental Health Dataset.csv`) | pending |
