...
* **Fase 4.** Staging structure, cleaning, deduplication, normalization, `Timestamp` parsing, load order and transactions, and update policy are specified in [`staging.md`](specification/staging.md). The deduplication rule — two raw rows are the same staged record only if they agree on all 17 source columns — is [ADR-0008](decisions/0008-staging-deduplication-grain.md)'s decision; 290,051 records reach staging from the 292,364-row source file.
