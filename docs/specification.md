...
* **Deduplication.** The source file has no respondent identifier. Check A4 of the [legacy audit](audit/legacy-audit.md) found 2,313 exact duplicate rows across all 17 columns (matching the legacy's own count) and 194,641 duplicate rows on the 16 non-`Timestamp` columns — a far larger number, from only 97,723 distinct profiles across the file. Here, "one row per response" means one row per staged record; what counts as a duplicate for the pipeline is still a Fase 4 decision, now informed by these figures rather than by an unresolved check.
