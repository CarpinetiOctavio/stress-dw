...
| 7.1 | Among respondents with explicit stress recognition, a convergent symptom profile, or both, what proportion report having ever sought treatment (`treatment`, lifetime), broken down by resource availability (`care_options`, "Yes" and "Not sure" reported separately; respondents reporting "No" are outside this question's scope), by explicit stress recognition (`Growing_Stress`, all three levels), and by whether a convergent symptom profile is present? | Original wording used present-tense "deciden buscar tratamiento" (English: "decide to seek treatment") and folded `care_options` into "≠ No"; the inclusion-path clause was later replaced by two direct cuts (ADR-0004, Amended). |
...
| 12 | Stress/resource-access rate | All records | Distribution over the three `care_options` levels (Yes / Not sure / No); denominator = the cell's respondents | `growing_stress` level × country × occupation × gender | Partitioned instead of filtered to "Yes" (F5); "Yes" and "Not sure" reported separately instead of folded, and "No" reported as its own level (F6, ADR-0005). |
...
| 15 | Care-options-to-treatment conversion rate | `(explicit_recognition OR symptom_cluster) AND care_options IN ('Yes','Not sure')` | Lifetime treatment rate | `growing_stress` level (3) × `symptom_cluster` × `care_options` (Yes / Not sure) | Resolves both problems F3 identified for this indicator, not only the "Not sure" fold; the inclusion-path tag was replaced by two direct cuts because it folded "Maybe" with "No" (ADR-0003, Addendum). |
| 16 | Treatment non-uptake rate under full context | Same population as 15 | Lifetime treatment rate and its complement; headline cell = `growing_stress` "Yes" × `symptom_cluster` × `care_options` "Yes" × `mental_health_interview` "Yes" | `growing_stress` level × `symptom_cluster` × `care_options` × `mental_health_interview` (3 levels) | Name asserts no claim of postponement — no time-to-treatment variable exists; grounded in Andrade et al. 2014 on settled self-reliance as the more common pattern (ADR-0003). |

`symptom_cluster` (`mood_swings IN ('Medium','High') AND coping_struggles='Yes' AND days_indoors IN ('15-30 days','31-60 days','More than 2 months')`) and `explicit_recognition` (`growing_stress='Yes'`) are the two components that replace the legacy's single OR'd `Indicador_Inferido_Estrés` flag. Neither the collapsed flag nor any of indicators 14–16 individually needs it stored as one column; both components are computed at query time, joining `dim_symptoms` and `dim_isolation` through the fact table (the dimension design is recorded in [ADR-0007](0007-person-grain-fact-table-and-dimension-grain-rule.md)).
