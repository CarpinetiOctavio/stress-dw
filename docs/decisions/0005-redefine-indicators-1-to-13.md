...
* `symptom_cluster`'s lack of external clinical validation (`conceptual-framework.md` §7) is already established and does not need to be re-argued here. Its validity caveat is written once, in the [`symptom_cluster` definition](../specification/definitions.md#symptom_cluster), and every point that defines or uses `symptom_cluster` references it there, so that it is neither assumed to carry over nor restated ([writing conventions, rule 4](../writing-conventions.md#4-introduce-before-use-one-home-per-concept)).
...
**Indicators 12, 13 — stress and resource access.** Population no longer filtered to `Growing_Stress = 'Yes'` — partitioned by all three levels (F5). `care_options = 'Yes'` and `care_options = 'Not sure'` reported separately instead of folded into "has access" (F6) — the same correction F3 established for indicator 15, applied here for the first time to 12 and 13. Country × occupation × gender kept as cuts, per Q6. Reported as the distribution over all three `care_options` levels (`'Yes'`, `'Not sure'` and `'No'`) within each `Growing_Stress` × country × occupation × gender cell, the denominator being the cell's full respondent count — in keeping with the all-levels rule applied to `Growing_Stress` and `Social_Weakness`, so that no level is left implicit (the shares also add to 100%).
...
* Constraint: every point in `docs/specification.md` that defines or uses `symptom_cluster` references its validity caveat at the single home of its definition ([`definitions.md`](../specification/definitions.md#symptom_cluster)); the caveat is not restated.
